...
DHT dht(DHT_data,DHT11);
 ...
 //για AES CRC
const unsigned char key[] = "bbbbbbbbbbbbbbbb";//{0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
...
unsigned char prev_iv[] ={0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x42};
...
void loop() {
 ...
 if(!fire){ 
  ...
  //οταν διαβασει καρτα ο μεσα καρταναγνώστης που ειναι για εξοδο
...
void OnDataRecv(const esp_now_recv_info_t * info, const uint8_t* incomingData, int len){
  ...
  if(memcmp(senderMacAddress, broadcastAddress, 6) == 0){
    if(strReceivedData=="1"){
       digitalWrite(security,LOW);
    
    }
    else if (strReceivedData=="0") {
      digitalWrite(security,HIGH);
    }
    else{
